    ...
    <Slider {...settings}>
      {["", ""].map((banner, index) => (
        <div key={index} className="flex relative h-[60vh]">
          ...
          <Image
            className="relative"
            src={"https://picsum.photos/800" || process.env.NEXT_PUBLIC_API + banner.image?.data?.attributes?.url}
            alt={banner.Name}
            fill
            priority={true}
            placeholder='blur'
            blurDataURL={process.env.NEXT_PUBLIC_API + banner.image?.data?.attributes?.url}
          />
