...
export default function ArticleGridCard({ name, img, description, vote = () => { } }) {
  const { user } = useAuth();
  return (
    <div className="max-h-1/2 text-left rounded-3xl h-full"
      style={{
    ...
    >
      ...
      {user ?
        <button className="font-extrabold min-w-24 px-4 h-8 bg-red-600 rounded-3xl" onClick={vote}>Votar</button>
        :
        <button className="font-extrabold min-w-24 px-4 h-8 bg-red-600 rounded-3xl" onClick={() => {
